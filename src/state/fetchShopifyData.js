...
export const queryCollection = (collectionHandle, numOfItems) =>
  client.graphQLClient.query([sortKey], (root) => {
    root.add(
      "collectionByHandle",
...
export const handleUpdatingProducts = (
  reduxKey,
  products,
  dispatch,
  productsFromRedux
...
export const fetchProductCollectionAction = (
  collectionHandle,
  numOfItems,
  handleReduxDispatch,
  collectionFromRedux
  ...
  return (dispatch) => {
    // set pending to true in redux
    dispatch(fetchPending());
    client.graphQLClient
      .send(queryCollection(collectionHandle, numOfItems))
      .then(({ model, data }) => {
        // take collectionHandle and format it for redux
        // e.g. - it as "featuredProducts" instead of something like "featured-products"
        const splitHandle = collectionHandle.split("-");
        const reduxKey =
          splitHandle[0] +
          splitHandle[1][0].toUpperCase() +
          splitHandle[1].slice(1);

        // get product nodes
        const productNodes = data.collectionByHandle.products.edges.map(
          (product) => product.node
        );

        // if reduxKey === featuredProducts, filter out products that aren't available for sale
        const products = reduxKey === 'featuredProducts' 
          ? productNodes.filter(product => product.availableForSale)
          : productNodes;
